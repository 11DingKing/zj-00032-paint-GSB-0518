...
function renderOverlay() {
  ...
  if (isDrawingShape.value) {
    const sx = shapeStartX.value;
    const sy = shapeStartY.value;
    const ex = shapeEndX.value;
    const ey = shapeEndY.value;
    const strokeColor = settingsStore.shapeStrokeColor;
    const fillColor = settingsStore.shapeFillColor;
    const lineWidth = settingsStore.shapeLineWidth;

    ctx.lineWidth = lineWidth;
    ctx.strokeStyle = colorToString(strokeColor);
    ctx.fillStyle = colorToString(fillColor);

    const shapeType = settingsStore.shapeTool;
...
function commitShape() {
  ...
  const pw = project.value.width;
  const ph = project.value.height;

  const layerCanvas = createCanvas(pw, ph);
  ...
  layerCtx.strokeStyle = colorToString(strokeColor);
  layerCtx.fillStyle = colorToString(fillColor);
  ...
  currentLayer.value.bitmapData = layerCtx.getImageData(0, 0, pw, ph);

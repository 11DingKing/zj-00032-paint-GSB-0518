...
export const useSettingsStore = defineStore("settings", () => {
  ...
  const shapeFillColor = ref<Color>({ r: 255, g: 255, b: 255, a: 0 });
  const shapeStrokeWidth = ref(3);
  const shapeUseFill = ref(false);
  ...
  function setShapeStrokeWidth(width: number): void {
    shapeStrokeWidth.value = Math.max(1, Math.min(100, width));
  }

  function setShapeUseFill(use: boolean): void {
    shapeUseFill.value = use;
  }
